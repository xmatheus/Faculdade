			...
			if ( *flag ) { //Se ocorreu a descida como esperado, então insere o valor no nó 
				if ( no->contChaves < constante->MAX_CHAVES) { //Verifica se há espaço no nó
					...
					*flag = 0;
                    ...
                    //Atualiza o encadeamento entre os possíveis nós folha, o antigo e o novo criado pela cisão
                    if ( (no->folha == 1) && (noAux->folha == 1) ){//Verifica se são nós folha:
                        noAux->prox = no->prox;
                        no->prox = noAux;
                    }
...
void CriarConstante(int T, tipo_constante **constante){//cria as 'constantes' para a criacao dinamica da arvore
	*constante = (tipo_constante*)malloc(sizeof(tipo_constante));
	(*constante)->T = T;
	(*constante)->MAX_FILHOS = 2*T;			//Máximo de registros
	(*constante)->MAX_CHAVES = 2 * T - 1;	//Máximo de filhos
	(*constante)->MIN_CHAVES = T - 1 ;   	//Mínimo de registros
}
...
void Cadeada(tipo_arvore *arvore){//imprime pela lista encadeada
	tipo_arvore *aux = arvore->filhos[0];
	int i =0;
	if(arvore != NULL){
		...
		while(aux != NULL){
			while(i<aux->contChaves){
				printf("[%d][%s]\n",aux->chaves[i]->valor,aux->chaves[i]->nome);
				i++;
			}
			aux = aux->prox;
			i=0;
